...
'''
How many units of sand come to rest before sand blocks the opening?
'''
...
while unitsOfSand < sandLimit:
    ...
    if topOfSand == maxY or \
        (topOfSand == 0 and map[topOfSand + 1][x-1] != '.' and map[topOfSand + 1][x+1] != '.'):
        map[topOfSand][x] = 'o'
        unitsOfSand += 1
        break
    ...
    map[topOfSand][x] = 'o'
    unitsOfSand += 1
    if unitsOfSand in [1, 22, 92]:
        log.debug(f'{unitsOfSand}:\n  {minX}\n' + printMap(map))

log.info(f'{unitsOfSand}:\n  {minX}\n' + printMap(map))
print(f'{unitsOfSand} units of sand come to rest before sand starts flowing into the abyss below.')
